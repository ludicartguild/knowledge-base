...
| Term | Meaning |
| --- | --- |
| SPA | Single-Page Application; one HTML load, then JavaScript handles updates and navigation. |
| Component | A self-contained, reusable piece of UI with its own markup and logic. |
| Props | Data passed into a component from its parent; read-only within the component. |
| State | Data a component owns and can change, triggering a re-render when updated. |
| Hook | A function (`useState`, `useEffect`, etc.) giving function components access to state and other React features. |
| JSX | HTML-like syntax embedded in JavaScript, compiled into React component output. |
| TypeScript | A typed superset of JavaScript, checked at compile time, compiled down to plain JavaScript. |
