...
| --- | --- |
| **Information Hiding** (Parnas, 1972) | A **design decision** to conceal the implementation details of a module behind an interface, so that the details can change without affecting callers. |
| **Encapsulation** | Bundling **data and the operations on that data** into a single unit (class/object), and restricting direct access to the data so that the operations are the only sanctioned way to modify it. |
| **Access Modifiers** (`private`, `protected`) | A **language enforcement mechanism** for information hiding. They help, but they are not encapsulation. A class that exposes a `get_x()` / `set_x()` pair for every field is technically using `private` while violating encapsulation in spirit. |
...
| Smell | Description |
| --- | --- |
| **Anemic Domain Model** | Classes that are pure data bags (fields + getters/setters, no behaviour). All business logic lives in external service classes that manipulate those bags. Named by Martin Fowler. Encapsulation is fully absent. |
| **Getter/Setter proliferation** | A `private` field with a public getter **and** a public setter is a public field with extra indirection. If callers can read and overwrite any field freely, the class has no control over its own state. |
| **Feature Envy** | A method that spends most of its time reaching into the data of **another** class. Named in _Refactoring_ (Fowler). Signals the method belongs on the class whose data it envies. |
| **Inappropriate Intimacy** | Two classes that reach deeply into each other’s internals, creating bidirectional content coupling. Either merge them or introduce an interface boundary. |
| **Returning raw mutable collections** | `def get_items(self) -> list[Item]` returns the internal list directly. Callers can mutate it, bypassing all invariant checks. Return a copy or an immutable view. |
