...
* [.line-through]#One class = one method.# SRP isn't about size; it's about reasons to change.
* [.line-through]#One class = one thing it does.# Too vague — almost any class can be described as doing "one thing" at some level of abstraction.
...
. *Preconditions cannot be strengthened* — the subtype can't demand more of the caller than the supertype did.
. *Postconditions cannot be weakened* — the subtype must deliver at least as much as the supertype promised.
. *Invariants of the supertype must be preserved* — any property that's always true of the supertype must also always be true of the subtype.
. *History rule* — the subtype cannot allow state changes that the supertype's contract didn't allow (e.g. making an immutable type mutable).
...
* Subclass methods that throw `UnsupportedOperationException` / `NotImplementedError` — the subclass can't honour the supertype's contract. (This is also an link:#_i_interface_segregation_principle_isp[ISP] smell.)
...
* xref:ROOT:software-architecture/principles/coupling-and-cohesion.adoc[*Coupling & Cohesion*] — SRP and ISP are direct attacks on coupling; DIP redirects it; OCP and LSP make it survivable as the system grows.
* xref:ROOT:software-architecture/principles/GRASP.adoc[*GRASP*] — High Cohesion is the GRASP-era precursor to SRP; Polymorphism precedes LSP and OCP; Protected Variations + Indirection precede DIP. GRASP guides the original assignment; SOLID audits the result.
* xref:ROOT:software-architecture/principles/DRY.adoc[*DRY*] — duplication crossing actor boundaries is often an SRP violation in disguise.
* xref:ROOT:software-architecture/principles/KISS.adoc[*KISS*] / xref:ROOT:software-architecture/principles/YAGNI.adoc[*YAGNI*] — OCP wants extensibility points; KISS and YAGNI restrict you to *real* variation, not imagined variation.
* xref:ROOT:software-architecture/principles/CQS.adoc[*CQS*] — a method-level axis of separation orthogonal to SRP's actor-level axis. Both reduce coupling.
