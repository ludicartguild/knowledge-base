...
. *Branch* — create a new branch off the main branch so the change is isolated: `git switch -c feature/add-login-form`.
. *Commit* — make changes and commit them in small, logical steps.
. *Push* — push the branch to GitHub with `git push`.
. *Open a Pull Request (PR)* — propose merging the branch into main. A PR shows the full diff and gives teammates a place to leave comments.
. *Review* — teammates read the code, ask questions, request changes, or approve it. This is where most feedback and learning happens.
. *Merge* — once approved (and once any automated checks pass — see xref:ROOT:full-stack/cicd-and-github-actions.adoc[CI/CD and GitHub Actions]), the PR is merged into main and the feature becomes part of the shared codebase.
...
| Repository
| A project's tracked files and full change history.
...
| Pull Request (PR)
| A proposal to merge one branch into another, with room for review.

| Merge conflict
| A case Git can't auto-resolve, requiring manual reconciliation.
|===

See xref:ROOT:full-stack/glossary.adoc[the glossary] for more terms across all foundational notes.

== Watch

video::tRZGeaHPoaw[youtube]

== Related notes

* xref:ROOT:full-stack/cicd-and-github-actions.adoc[CI/CD and GitHub Actions] — what happens automatically when a PR is opened or merged.
* xref:ROOT:full-stack/communication.adoc[Communication] — how to talk about knowledge gaps and workflow in an interview.
* xref:ROOT:full-stack/glossary.adoc[Glossary] — definitions for terms used across these notes.
